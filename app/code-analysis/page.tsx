  ...
  return (
    <>
      ...
      {pageTab === "learn" && (() => {
        const LESSONS = [
          {
            id: 0, icon: "🧩", color: "#f59e0b",
            title: "What is an AI Agent System?",
            subtitle: "From chatbot to autonomous worker",
            concept: "An AI Agent System is a program where a language model (LLM) doesn't just answer questions — it decides what actions to take, executes those actions using tools, observes the results, and loops until a goal is achieved.",
            analogy: "Think of it like a LEGO set. The LLM is the brain (decision brick). Tools are special LEGO pieces — a web-search piece, a calculator piece, a database piece. The orchestrator is the instruction manual that snaps the bricks together. You can swap any brick without rebuilding the whole set.",
            diagram: [
              { label:"User Goal",  arrow:true, color:"#f59e0b" },
              { label:"Orchestrator", arrow:true, color:"#e879f9" },
              { label:"LLM (Brain)", arrow:true, color:"#4f8ef7" },
              { label:"Tools",      arrow:true, color:"#34d399" },
              { label:"Observe",    arrow:true, color:"#a78bfa" },
              { label:"Result ✅",  arrow:false, color:"#34d399" },
            ],
            concepts: [
              { term:"LLM", def:"Large Language Model. The reasoning core — reads context, decides next action.", color:"#4f8ef7" },
              { term:"Tool", def:"A function the LLM can call: search, read file, query DB, send email.", color:"#34d399" },
              { term:"Agent", def:"LLM + tools + memory + a loop that keeps acting until goal is done.", color:"#a78bfa" },
              { term:"Orchestrator", def:"The code that manages the loop: calls LLM → parses tool call → executes → feeds result back.", color:"#e879f9" },
              { term:"ReAct Loop", def:"Reason → Act → Observe. The fundamental cycle every agent runs.", color:"#fb923c" },
            ],
            code: `// The simplest possible agent loop
async function runAgent(goal: string) {
  ...
  while (true) {
    ...
    // 2. ACT — if LLM wants a tool, run it
    if (response.toolCall) {
      const result = await tools[response.toolCall.name](response.toolCall.args);
      context.push({ role: "tool", content: result });
    } else {
      // 3. DONE — LLM gave a final answer
      return response.text;
    }
  }
}`,
            stacks: [
              { layer:"LLM", options:["GPT-4o (OpenAI)","Gemini 2.5 (Google)","Claude 3.5 (Anthropic)","Llama 3 (Meta / Ollama local)","DeepSeek V3 (open-source)"], rec:"Gemini 2.5 Flash for speed+cost, Claude for reasoning" },
              { layer:"SDK", options:["Vercel AI SDK","LangChain","LlamaIndex","Semantic Kernel","Raw API"], rec:"Vercel AI SDK if you're on Next.js" },
            ],
            quiz: [
              { q:"What makes an agent different from a regular chatbot?", a:"An agent has tools it can call, a loop that keeps acting, and observes results — not just one-shot replies." },
              { q:"Name the 3 steps in a ReAct loop.", a:"Reason (decide what to do), Act (call a tool), Observe (read the result back)." },
            ],
            fileTree: `my-agent/
├── src/
│   ├── agent.ts          ← orchestrator loop
│   ├── tools/
│   │   └── search.ts     ← one tool per file
│   ├── llm/
│   │   └── client.ts     ← LLM provider wrapper
│   └── index.ts          ← entry point
├── .env                  ← API keys (never commit)
└── package.json`,
          },
          {
            id: 1, icon: "🔧", color: "#34d399",
            title: "Tools — The LEGO Pieces",
            subtitle: "Giving your agent superpowers",
            concept: "A Tool is a typed function the LLM can choose to call. The LLM sees the name, description, and parameter schema. If it decides to use it, your code executes it and returns the result. Tools are the only way an agent touches the real world.",
            analogy: "Each tool is a LEGO piece with a specific shape (its TypeScript interface). A search-tool piece has a 'query' socket. A database-tool piece has 'table' and 'filter' sockets. The LLM snaps the right piece into its reasoning, your orchestrator actually runs it.",
            diagram: [
              { label:"LLM decides action", arrow:true, color:"#4f8ef7" },
              { label:"tool_call: { name, args }", arrow:true, color:"#fbbf24" },
              { label:"Tool Registry", arrow:true, color:"#34d399" },
              { label:"execute(args)", arrow:true, color:"#34d399" },
              { label:"result → LLM context", arrow:false, color:"#a78bfa" },
            ],
            concepts: [
              { term:"Tool Schema", def:"JSON Schema describing what parameters the tool accepts. LLM reads this to know how to call it.", color:"#34d399" },
              { term:"Tool Registry", def:"A map of { name → function }. Orchestrator looks up the function by name when LLM asks.", color:"#4f8ef7" },
              { term:"Deterministic Tool", def:"Always returns same output for same input — math, date formatting. Safe to cache.", color:"#a78bfa" },
              { term:"Side-effect Tool", def:"Mutates state — send email, write file, DB insert. Needs confirmation gates.", color:"#fb923c" },
            ],
            code: `// Define a tool with full type safety
import { z } from "zod";
...
export const searchWebTool = {
  ...
  execute: async ({ query, limit }) => {
    const results = await fetch(\`/api/search?q=\${query}&n=\${limit}\`);
    return results.json();
  },
};
...
export const TOOL_REGISTRY = {
  ...
  send_email:   sendEmailTool,   // ← side-effect, add confirm gate
};`,
            stacks: [
              { layer:"Tool Framework", options:["Vercel AI SDK tool()","LangChain Tool","OpenAI function_calling","Zod schemas (raw)"], rec:"Vercel AI SDK tool() — built-in streaming, Zod validation" },
              { layer:"Common Tools", options:["Tavily (web search)","E2B (code sandbox)","Browserbase (browser)","Neon (SQL)","Resend (email)"], rec:"Tavily for search, Neon for SQL if on Vercel" },
            ],
            quiz: [
              { q:"Why does a tool need a JSON Schema / Zod definition?", a:"The LLM reads the schema to know what parameters to pass. Without it the LLM can't call the tool correctly." },
              { q:"What is the difference between a deterministic and side-effect tool?", a:"Deterministic is safe to retry/cache. Side-effect mutates state (DB write, email) — needs confirmation gates and idempotency." },
            ],
            fileTree: `src/tools/
├── index.ts         ← exports TOOL_REGISTRY
├── search.ts        ← Tavily web search
├── readFile.ts      ← fs.readFile wrapper
├── queryDb.ts       ← Neon SQL queries
├── sendEmail.ts     ← Resend email (side-effect)
└── codeRun.ts       ← E2B sandbox execution`,
          },
          {
            id: 2, icon: "🧠", color: "#4f8ef7",
            title: "RAG — Give Your Agent Memory",
            subtitle: "Retrieval-Augmented Generation",
            concept: "RAG lets your agent query a knowledge base of your own documents instead of relying only on its training data. Documents are chunked, embedded into vectors, stored in a vector DB. At query time, the most relevant chunks are retrieved and injected into the LLM's context.",
            analogy: "Imagine your agent has a filing cabinet (vector DB). Each document is cut into index cards (chunks), each card gets a GPS coordinate (embedding vector). When the agent needs info, it describes what it's looking for, and the nearest GPS coordinates (cosine similarity) pull the right cards from the cabinet.",
            diagram: [
              { label:"📄 Your Docs", arrow:true, color:"#fbbf24" },
              { label:"Chunker (800 chars)", arrow:true, color:"#fb923c" },
              { label:"Embedder (text→vector)", arrow:true, color:"#4f8ef7" },
              { label:"Vector DB (store)", arrow:false, color:"#34d399" },
              { label:"Query time ↓", arrow:true, color:"#5c6480" },
              { label:"Embed query → similarity search", arrow:true, color:"#4f8ef7" },
              { label:"Top-k chunks → LLM context", arrow:false, color:"#a78bfa" },
            ],
            concepts: [
              { term:"Embedding", def:"A list of ~1500 numbers that represent the meaning of text. Similar meanings → nearby vectors.", color:"#4f8ef7" },
              { term:"Cosine Similarity", def:"Measures angle between two vectors. 1.0 = identical meaning, 0 = unrelated. Used to rank chunks.", color:"#34d399" },
              { term:"Chunk", def:"A fixed-size slice of a document (e.g. 800 chars, 120 char overlap). Unit of retrieval.", color:"#a78bfa" },
              { term:"Top-k Retrieval", def:"Return the k most similar chunks. k=5 is typical — enough context, not too many tokens.", color:"#fb923c" },
              { term:"Hybrid Search", def:"Keyword filter (fast O(n)) → vector re-rank (slow but smart). Best of both worlds.", color:"#38bdf8" },
            ],
            code: `// RAG pipeline — ingest + retrieve
import { GoogleGenerativeAI } from "@google/generative-ai";

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY!);

// INGEST: embed a document chunk
...
async function retrieve(query: string, chunks: Chunk[], k = 5) {
  const qVec = await embedChunk(query);
  return chunks
    ...
    .slice(0, k);
            ...
            stacks: [
              { layer:"Embedding Model", options:["text-embedding-004 (Google, free)","text-embedding-3-small (OpenAI)","nomic-embed-text (Ollama, local)","mxbai-embed-large (local)"], rec:"text-embedding-004 — 768 dims, generous free tier" },
              { layer:"Vector DB", options:["pgvector (Neon — you already have it)","Pinecone (managed, scale)","Chroma (local dev)","Weaviate (open source)","Qdrant (Rust, fast)"], rec:"pgvector on Neon if you're already on Postgres" },
            ],
            quiz: [
              { q:"Why do we chunk documents before embedding?", a:"LLMs have token limits. Chunking keeps each piece retrievable — you pull only relevant chunks, not the entire doc." },
              { q:"What does cosine similarity measure?", a:"The angle between two embedding vectors. 1.0 = same meaning. Used to rank which chunks are most relevant to a query." },
            ],
            fileTree: `src/rag/
├── ingest.ts       ← chunk + embed + store
├── retrieve.ts     ← embed query + cosine rank
├── chunker.ts      ← sliding window 800/120
├── embedder.ts     ← Gemini text-embedding-004
└── vectorStore.ts  ← Neon pgvector or in-memory`,
          },
          {
            id: 3, icon: "🔌", color: "#38bdf8",
            title: "MCP — The Universal Tool Plug",
            subtitle: "Model Context Protocol by Anthropic",
            concept: "MCP (Model Context Protocol) is an open standard that separates tool servers from agent code. Instead of hard-coding tools into your agent, you run a lightweight MCP server that exposes tools over a standard protocol. Any MCP-compatible agent connects and gets those tools automatically — no code changes needed.",
            analogy: "Think of it like a USB standard. Before USB, every device needed its own special port. MCP is the USB port for AI tools. Your agent is the laptop — it just needs one USB port (MCP client). Your tools are devices — they just need a USB connector (MCP server). Plug any tool into any agent.",
            diagram: [
              { label:"Agent (MCP Client)", arrow:true, color:"#4f8ef7" },
              { label:"MCP Protocol (JSON-RPC)", arrow:true, color:"#38bdf8" },
              { label:"MCP Server", arrow:false, color:"#34d399" },
              { label:"Tool A | Tool B | Tool C", arrow:false, color:"#a78bfa" },
            ],
            concepts: [
              { term:"MCP Server", def:"A process that exposes tools over JSON-RPC. Written once, used by any agent.", color:"#38bdf8" },
              { term:"MCP Client", def:"Built into your agent. Connects to MCP servers, discovers tools, calls them.", color:"#4f8ef7" },
              { term:"Tool Discovery", def:"Client calls tools/list → server returns all available tools with schemas. Dynamic — no hardcoding.", color:"#34d399" },
              { term:"Transport", def:"How client↔server talk: stdio (local process), SSE (network), WebSocket.", color:"#a78bfa" },
              { term:"Resources", def:"MCP also exposes read-only data sources (files, DB rows) via resources/read.", color:"#fb923c" },
            ],
            code: `// Build an MCP server in ~30 lines
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
...
// Register a tool — same Zod schema you already know
server.tool(
  "get_weather",
...
// Start server (stdio transport = local process)
const transport = new StdioServerTransport();
await server.connect(transport);
console.error("MCP server running on stdio");`,
            stacks: [
              { layer:"MCP SDK", options:["@modelcontextprotocol/sdk (official)","FastMCP (Python)","mcp-framework (TS community)"], rec:"Official SDK — always up to date with spec" },
              { layer:"Transport", options:["stdio (local subprocess)","SSE (Vercel Edge / serverless)","WebSocket (persistent connection)"], rec:"SSE for Vercel, stdio for local" },
              { layer:"MCP Hosts", options:["Claude Desktop","Claude Code CLI","Cursor","VS Code Copilot","Custom agent"], rec:"Claude Code supports MCP natively — test there first" },
            ],
            quiz: [
              { q:"What problem does MCP solve?", a:"Tool fragmentation — before MCP every agent needed custom tool code. MCP standardises the interface so any tool works with any agent." },
              { q:"What transport should you use for a Vercel-hosted MCP server?", a:"SSE (Server-Sent Events) — stateless, works with serverless/edge functions." },
            ],
            fileTree: `mcp-server/
├── src/
│   ├── server.ts      ← McpServer setup
│   ├── tools/
│   │   ├── weather.ts ← one tool per file
│   │   └── stock.ts
│   └── index.ts       ← entry point
├── package.json
└── .env               ← tool API keys`,
          },
          {
            ...
            subtitle: "Google's Agent-to-Agent Protocol",
            concept: "A2A (Agent-to-Agent) is Google's open protocol for agents to discover and communicate with other agents across different frameworks, vendors, and clouds. An agent publishes an Agent Card (JSON manifest). Other agents read the card, send Tasks, and get streaming Updates back.",
            analogy: "Like hiring a contractor. You don't care how the contractor works internally — you just hand them a task description (Task), agree on deliverables (AgentCard capabilities), and they send you progress updates (streaming Events). A2A is the standardised contract between contractors.",
            diagram: [
              { label:"Orchestrator Agent", arrow:true, color:"#e879f9" },
              { label:"GET /.well-known/agent.json", arrow:true, color:"#a78bfa" },
              { label:"AgentCard (capabilities)", arrow:true, color:"#a78bfa" },
              { label:"POST /tasks/send { goal }", arrow:true, color:"#38bdf8" },
              { label:"Sub-Agent executes", arrow:true, color:"#34d399" },
              { label:"Streaming updates → done", arrow:false, color:"#34d399" },
            ],
            concepts: [
              { term:"AgentCard", def:"JSON manifest at /.well-known/agent.json. Describes what the agent can do, its endpoint, auth.", color:"#a78bfa" },
              { term:"Task", def:"A unit of work sent from one agent to another. Has id, goal, context.", color:"#38bdf8" },
              { term:"Streaming Events", def:"The sub-agent sends back incremental updates as it works. Client subscribes like SSE.", color:"#34d399" },
              { term:"Skills", def:"Named capabilities declared in AgentCard — e.g. 'audit-code', 'summarize-doc'.", color:"#fb923c" },
              { term:"Multi-Agent Network", def:"Orchestrator + N specialist sub-agents, each with their own A2A endpoint.", color:"#e879f9" },
            ],
            code: `// A2A sub-agent skeleton (Express)
import express from "express";

const app = express();
app.use(express.json());

// 1. Publish the Agent Card
app.get("/.well-known/agent.json", (_, res) => res.json({
  name: "code-audit-agent",
  version: "1.0.0",
  skills: [{ id: "audit", description: "Security-audit TypeScript files" }],
  endpoint: "https://audit-agent.example.com",
}));

// 2. Receive tasks from orchestrator
app.post("/tasks/send", async (req, res) => {
  const { id, goal, context } = req.body;

  // Stream back updates via SSE
  res.setHeader("Content-Type", "text/event-stream");
  res.write(\`data: {"status":"working","taskId":"\${id}"}\n\n\`);

  const result = await runAudit(goal, context);

  res.write(\`data: {"status":"done","output":\${JSON.stringify(result)}}\n\n\`);
  res.end();
});

app.listen(3001);`,
            stacks: [
              { layer:"A2A SDK", options:["google/a2a (official Python/TS)","Custom HTTP (just JSON)","gRPC (high throughput)"], rec:"Start with custom HTTP — A2A is just conventions over JSON" },
              { layer:"Hosting Sub-agents", options:["Vercel Functions","Cloud Run (Google)","Railway","Fly.io","AWS Lambda"], rec:"Cloud Run for long-running, Vercel Functions for quick tasks" },
            ],
            quiz: [
              { q:"What is an AgentCard?", a:"A JSON manifest published at /.well-known/agent.json that describes the agent's capabilities, skills, and endpoint URL." },
              { q:"How does an orchestrator know what a sub-agent can do?", a:"It fetches the AgentCard — like reading a job description before assigning work." },
            ],
            fileTree: `agents/
├── orchestrator/
│   ├── agent.ts       ← discovers + delegates via A2A
│   └── agentCard.json ← this agent's own manifest
├── audit-agent/
...
└── summarize-agent/
    ...
    └── agentCard.json`,
          ...
          {
            id: 5, icon: "🎼", color: "#e879f9",
            title: "Orchestration — The Conductor",
            subtitle: "Chaining agents, models, and tools",
            concept: "Orchestration is the code that decides which agent/tool/model to call in what order, passes context between them, handles errors, and returns the final result. It's the difference between one agent and a capable system.",
            analogy: "An orchestrator is like a music conductor. It doesn't play an instrument — it knows the score (workflow), cues each musician (tool/agent) at the right time, handles a player hitting a wrong note (retry/fallback), and ensures the concert ends with a standing ovation (correct output).",
            diagram: [
              { label:"User Request", arrow:true, color:"#fbbf24" },
              { label:"Orchestrator (pipeline.ts)", arrow:false, color:"#e879f9" },
              { label:"Phase 1: Analyse", arrow:true, color:"#4f8ef7" },
              { label:"Phase 2: Retrieve (RAG)", arrow:true, color:"#34d399" },
              { label:"Phase 3: Act (Tools)", arrow:true, color:"#fb923c" },
              { label:"Phase 4: Synthesise → Response", arrow:false, color:"#e879f9" },
            ],
            concepts: [
              { term:"Sequential Chain", def:"Steps run one after another. Each step's output is the next step's input. Simple and predictable.", color:"#4f8ef7" },
              { term:"Parallel Fan-out", def:"Multiple sub-tasks run concurrently with Promise.all(), results merged. Faster for independent tasks.", color:"#34d399" },
              { term:"Conditional Routing", def:"Switch on content type / user role / confidence score to pick the right branch.", color:"#a78bfa" },
              { term:"Retry + Fallback", def:"On tool error: retry N times, then fallback to simpler path. Never crash the whole pipeline.", color:"#fb923c" },
              { term:"Context Passing", def:"Each step enriches a shared context object. Downstream steps read earlier results.", color:"#e879f9" },
            ],
            code: `// Multi-phase orchestration pipeline
interface PipelineContext {
  query:    string;
  chunks:   string[];        // from RAG
  toolResults: unknown[];    // from tools
  finalAnswer: string;
}

async function pipeline(query: string): Promise<string> {
  const ctx: PipelineContext = { query, chunks: [], toolResults: [], finalAnswer: "" };

  // Phase 1 — parallel: retrieve + classify
  const [chunks, intent] = await Promise.all([
    retrieve(ctx.query),              // RAG
    classify(ctx.query),              // fast small model
  ]);
  ctx.chunks = chunks.map(c => c.text);

  // Phase 2 — conditional: tool use if needed
  if (intent === "realtime") {
    ctx.toolResults = await searchWeb(ctx.query);
  }

  // Phase 3 — synthesise with full context
  ctx.finalAnswer = await llm.complete({
    system:  "Answer using ONLY the context below.",
    context: [...ctx.chunks, ...ctx.toolResults.map(String)].join("\n\n"),
    query:   ctx.query,
  });
  ...
  return ctx.finalAnswer;
            ...
            stacks: [
              { layer:"Orchestration Framework", options:["Custom TypeScript (full control)","Vercel AI SDK streamText","LangGraph (stateful)","CrewAI (Python, roles)","AutoGen (Microsoft)"], rec:"Custom TS for full control, LangGraph for complex state machines" },
              { layer:"Workflow Persistence", options:["Vercel KV (Redis)","Inngest (durable functions)","Temporal (enterprise)","Upstash Redis"], rec:"Inngest for Vercel — retries, events, durable execution built in" },
            ],
            quiz: [
              { q:"When would you use parallel fan-out instead of sequential chain?", a:"When steps are independent (no data dependency) — e.g. retrieve docs AND classify intent simultaneously. Cuts latency significantly." },
              { q:"What should happen when a tool call fails?", a:"Retry N times with exponential backoff, then fall back to a simpler path — never let one tool failure crash the entire pipeline." },
            ],
            fileTree: `src/orchestration/
├── pipeline.ts       ← main entry: chains all phases
├── phases/
│   ├── retrieve.ts   ← Phase 1: RAG
│   ├── classify.ts   ← Phase 2: intent routing
│   ├── toolUse.ts    ← Phase 3: tool fan-out
│   └── synthesise.ts ← Phase 4: final answer
└── context.ts        ← shared PipelineContext type`,
          },
          {
            id: 6, icon: "⚙️", color: "#fb923c",
            title: "Context Engineering",
            subtitle: "The craft of filling the context window",
            concept: "Context engineering is the discipline of deciding exactly what text to put into the LLM's context window on every call — system prompt, retrieved docs, tool results, conversation history, and user message — in the right order and size to maximise answer quality without hitting token limits.",
            analogy: "An LLM's context window is a whiteboard. Context engineering is the art of knowing what to write on it, what to erase, and in what order. Write too little — the LLM hallucinates. Write too much — it ignores the important parts or errors out. Write it wrong — even a smart LLM fails.",
            diagram: [
              { label:"System Prompt (persona + rules)", arrow:false, color:"#4f8ef7" },
              { label:"Memory (past conversation summary)", arrow:false, color:"#a78bfa" },
              { label:"Retrieved Chunks (RAG top-k)", arrow:false, color:"#34d399" },
              { label:"Tool Results (latest actions)", arrow:false, color:"#fb923c" },
              { label:"User Message", arrow:true, color:"#fbbf24" },
              { label:"LLM Context Window (128k tokens)", arrow:false, color:"#e879f9" },
            ],
            concepts: [
              { term:"System Prompt", def:"Sets the AI's persona, rules, and output format. Injected at the start of every call.", color:"#4f8ef7" },
              { term:"Token Budget", def:"128k context ≈ 96k words. Reserve 20% for output. Allocate the rest across sections.", color:"#fb923c" },
              { term:"Context Compression", def:"Summarise older messages to free tokens. Keep last 10 turns verbatim, earlier as summary.", color:"#a78bfa" },
              { term:"Injection Order", def:"Most relevant content closest to the user message — LLMs attend more strongly to nearby tokens.", color:"#34d399" },
              { term:"Few-shot Examples", def:"2-3 worked examples in the system prompt show the LLM the exact output format you want.", color:"#38bdf8" },
            ],
            code: `// Context assembly — budget-aware
const TOKEN_BUDGET = 100_000;     // reserve 28k for output
...
function buildContext(opts: {
  ...
  let used = countTokens(opts.system) + countTokens(opts.query);

  // Priority 1 — system prompt (always included)
  messages.push({ role: "system", content: opts.system });

  // Priority 2 — RAG chunks (most relevant first)
  const chunkBudget = Math.floor((TOKEN_BUDGET - used) * 0.4);
  let chunkTokens = 0;
  for (const chunk of opts.chunks) {
    const t = countTokens(chunk);
    if (chunkTokens + t > chunkBudget) break;
    messages.push({ role: "system", content: \`[CONTEXT] \${chunk}\` });
    chunkTokens += t;
  }

  // Priority 3 — compressed history
  const historyBudget = TOKEN_BUDGET - used - chunkTokens - 2000;
  messages.push(...compressHistory(opts.history, historyBudget));

  // Priority 4 — user message (always last)
  messages.push({ role: "user", content: opts.query });
  return messages;
}`,
            stacks: [
              { layer:"Token Counter", options:["tiktoken (OpenAI, accurate)","@anthropic-ai/tokenizer","gemini countTokens API","Rough estimate: 1 token ≈ 4 chars"], rec:"Use provider's own counter — models differ on tokenisation" },
              { layer:"Memory / Compression", options:["Custom summarisation prompt","MemGPT / Letta","Zep (open source memory)","Redis for session state"], rec:"Start with custom summarisation prompt — simplest, works great" },
            ],
            quiz: [
              { q:"Why does injection order matter in a context window?", a:"LLMs pay more attention to content near the user message (recency bias). Put your most relevant retrieved chunks just before the user message." },
              { q:"What is context compression and why is it needed?", a:"Summarising old conversation turns to free tokens. Without it, long conversations hit the context limit and start dropping messages." },
            ],
            fileTree: `src/context/
├── builder.ts      ← assemble final context array
├── compress.ts     ← summarise old history
...
└── prompts/
    ...
    └── examples.ts ← few-shot examples`,
          ...
          {
            id: 7, icon: "🚀", color: "#4f8ef7",
            title: "Deploy — Zero to Production",
            subtitle: "Local → GitHub → CI/CD → Vercel → Docker",
            concept: "Deployment is the process of taking your local agent and making it reliably available to real users. Each stage adds reliability: version control, automated tests, cloud hosting, containerisation, and finally enterprise Kubernetes orchestration.",
            analogy: "Building a house: local dev = blueprint on paper. GitHub = blueprint in a fireproof safe. CI/CD = automatic building inspection on every change. Vercel = moving into the house. Docker = a container home that can move anywhere. Kubernetes = a whole neighbourhood that scales automatically.",
            diagram: [
              { label:"1. Local (npm run dev)", arrow:true, color:"#fbbf24" },
              { label:"2. GitHub (git push)", arrow:true, color:"#34d399" },
              { label:"3. CI/CD (GitHub Actions)", arrow:true, color:"#4f8ef7" },
              { label:"4. Vercel (preview → prod)", arrow:true, color:"#a78bfa" },
              { label:"5. Docker (containerise)", arrow:true, color:"#38bdf8" },
              { label:"6. K8s / Cloud Run (scale)", arrow:false, color:"#e879f9" },
            ],
            concepts: [
              { term:"Environment Variables", def:"API keys, DB URLs — never hardcode. Use .env locally, Vercel env vars in prod.", color:"#fb923c" },
              { term:"Preview Deployments", def:"Every PR gets its own live URL on Vercel. Test before merge.", color:"#4f8ef7" },
              { term:"GitHub Actions", def:"YAML workflow file that runs build, test, lint on every push. Blocks bad code.", color:"#34d399" },
              { term:"Docker Image", def:"Snapshots your app + all dependencies into a portable container.", color:"#38bdf8" },
              { term:"Health Check", def:"GET /api/health returns 200 OK. Load balancers ping it to know if pod is alive.", color:"#a78bfa" },
            ],
    ...
    steps:
      ...
      - uses: amondnet/vercel-action@v25
        with:
          ...
          vercel-args: '--prod'`,
            stacks: [
              { layer:"Hosting", options:["Vercel (Next.js native, serverless)","Railway (Dockerised, always-on)","Fly.io (global edge Docker)","Cloud Run (GCP, scale-to-zero)","AWS ECS (enterprise)"], rec:"Vercel for Next.js, Railway for always-on agents with long tasks" },
              { layer:"CI/CD", options:["GitHub Actions (free, 2000 min/month)","GitLab CI","CircleCI","Buildkite (enterprise)"], rec:"GitHub Actions — free, integrates with Vercel Actions" },
              { layer:"Secrets", options:["Vercel env vars","GitHub Actions secrets","HashiCorp Vault (enterprise)","AWS Secrets Manager"], rec:"Vercel env vars for app, GitHub secrets for CI pipeline" },
            ],
            quiz: [
              { q:"Where should API keys live in production?", a:"In platform environment variables (Vercel dashboard, not code). Never commit .env files. Rotate keys if they leak." },
              { q:"What is the purpose of a GitHub Actions CI check on every PR?", a:"Catches broken TypeScript, failing tests, and lint errors before they reach main — preventing bad code from reaching users." },
            ],
            fileTree: `# Full enterprise structure
my-ai-agent/
├── .github/
│   └── workflows/
│       └── deploy.yml   ← CI/CD pipeline
├── src/                 ← agent code (see prev lessons)
├── prisma/
│   └── schema.prisma    ← DB schema
├── Dockerfile           ← containerisation
├── docker-compose.yml   ← local dev with DB
├── .env.example         ← template (commit this)
├── .env                 ← real secrets (gitignore!)
└── package.json`,
          },
        ];

        const PATHS = [
          { id:"vercel",   label:"⚡ Vercel + Neon Stack",    color:"#4f8ef7", desc:"Next.js, Vercel AI SDK, Neon pgvector, Gemini. Best for rapid SaaS." },
          { id:"google",   label:"🌐 Google Cloud Full Stack", color:"#34d399", desc:"Gemini, Firebase, Vertex AI, Cloud Run. Best for Google ecosystem." },
          { id:"local",    label:"🏠 Local / Private AI",      color:"#a78bfa", desc:"Ollama, ChromaDB, LangChain. Full privacy, no cloud costs." },
          { id:"openai",   label:"🤖 OpenAI Classic Stack",    color:"#fb923c", desc:"GPT-4o, Pinecone, LangChain, Vercel. Most tutorials use this." },
          { id:"oss",      label:"🔓 Fully Open Source",       color:"#38bdf8", desc:"LLaMA 3, Qdrant, Haystack, FastAPI. Zero vendor lock-in." },
          { id:"enterprise",label:"🏢 Enterprise / Gov",       color:"#e879f9", desc:"Azure OpenAI, pgvector, Temporal, K8s. Air-gap capable." },
        ];

        const lesson = LESSONS[learnLesson];

        return (
          <div style={{display:"flex",height:"calc(100dvh - 96px)",background:"#0d0f1a",overflow:"hidden"}}>

            {/* ── LEFT SIDEBAR ── */}
            <div style={{width:240,flexShrink:0,borderRight:"1px solid #1a1d2e",overflowY:"auto",padding:"16px 12px",display:"flex",flexDirection:"column",gap:6}}>
              <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",padding:"4px 8px",marginBottom:4}}>8 CORE LESSONS</div>
              {LESSONS.map((l,i) => (
                <button key={l.id} onClick={() => setLearnLesson(i)} style={{
                  display:"flex",alignItems:"center",gap:9,padding:"9px 10px",borderRadius:8,textAlign:"left",
                  background: learnLesson===i ? `${l.color}18` : "transparent",
                  border: learnLesson===i ? `1px solid ${l.color}40` : "1px solid transparent",
                  cursor:"pointer",transition:"all 0.15s",
                }}>
                  <span style={{fontSize:16,flexShrink:0}}>{l.icon}</span>
                  <div style={{minWidth:0}}>
                    <div style={{fontSize:11.5,fontWeight:700,color: learnLesson===i ? l.color : "#8892b0",lineHeight:1.3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.title}</div>
                    <div style={{fontSize:10,color:"#4a5270",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.subtitle}</div>
                  </div>
                  {learnLesson===i && <ChevronRight size={10} style={{flexShrink:0,color:l.color,marginLeft:"auto"}}/>}
                </button>
              ))}

              <div style={{height:1,background:"#1a1d2e",margin:"8px 0"}}/>
              <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",padding:"4px 8px"}}>CHOOSE YOUR STACK</div>
              {PATHS.map(p => (
                <button key={p.id} onClick={() => setLearnPath(learnPath===p.id ? null : p.id)} style={{
                  display:"flex",alignItems:"flex-start",gap:8,padding:"8px 10px",borderRadius:7,textAlign:"left",
                  background: learnPath===p.id ? `${p.color}15` : "transparent",
                  border: learnPath===p.id ? `1px solid ${p.color}40` : "1px solid #1a1d2e",
                  cursor:"pointer",transition:"all 0.15s",
                }}>
                  <div style={{fontSize:11,fontWeight:700,color: learnPath===p.id ? p.color : "#7d88a8",lineHeight:1.4}}>{p.label}</div>
                </button>
              ))}

              {learnPath && (() => {
                const p = PATHS.find(x => x.id===learnPath)!;
                return (
                  <div style={{padding:"10px 12px",borderRadius:8,background:`${p.color}10`,border:`1px solid ${p.color}30`,marginTop:2}}>
                    <div style={{fontSize:11,color:p.color,fontWeight:700,marginBottom:4}}>{p.label}</div>
                    <div style={{fontSize:11,color:"#9aa3c0",lineHeight:1.6}}>{p.desc}</div>
                  </div>
                );
              })()}
            </div>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"24px 32px"}}>
              ...
              {/* Header */}
              <div style={{marginBottom:24,padding:"20px 24px",borderRadius:12,background:`linear-gradient(135deg,${lesson.color}10,${lesson.color}05)`,border:`1px solid ${lesson.color}25`}}>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
                  <span style={{fontSize:28}}>{lesson.icon}</span>
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:10,background:`${lesson.color}20`,border:`1px solid ${lesson.color}40`,color:lesson.color}}>LESSON {lesson.id + 1} / {LESSONS.length}</span>
                    </div>
                    <h1 style={{fontSize:22,fontWeight:800,color:"#eaedf8",margin:"4px 0 0"}}>{lesson.title}</h1>
                    <p style={{fontSize:13,color:lesson.color,margin:"2px 0 0",fontWeight:600}}>{lesson.subtitle}</p>
                  </div>
                </div>
                <p style={{fontSize:14,color:"#9aa3c0",lineHeight:1.75,margin:"8px 0 0"}}>{lesson.concept}</p>
              </div>

              {/* LEGO Analogy */}
              <div style={{marginBottom:20,padding:"14px 18px",borderRadius:10,background:"rgba(251,191,36,0.06)",border:"1px solid rgba(251,191,36,0.2)"}}>
                <div style={{fontSize:11,fontWeight:800,color:"#fbbf24",letterSpacing:"0.1em",marginBottom:7}}>🧩 LEGO ANALOGY</div>
                <p style={{fontSize:13.5,color:"#c9d1f0",lineHeight:1.75,margin:0}}>{lesson.analogy}</p>
              </div>

              {/* Architecture Diagram */}
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>🗺️ HOW IT FLOWS</div>
                <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:0,padding:"16px 20px",borderRadius:10,background:"#0a0c15",border:"1px solid #1a1d2e"}}>
                  {lesson.diagram.map((d,i) => (
                    <React.Fragment key={i}>
                      <div style={{padding:"6px 14px",borderRadius:7,background:`${d.color}15`,border:`1px solid ${d.color}35`,fontSize:12,fontWeight:700,color:d.color,whiteSpace:"nowrap"}}>
                        {d.label}
                      </div>
                      {d.arrow && <span style={{fontSize:14,color:"#2a2e46",margin:"0 6px",flexShrink:0}}>→</span>}
                    </React.Fragment>
                  ))}
                </div>
              </div>
              ...
              {/* Concepts grid */}
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>🧩 KEY CONCEPTS</div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>
                  {lesson.concepts.map((c,i) => (
                    <div key={i} style={{padding:"12px 14px",borderRadius:9,background:"#12141f",border:`1px solid ${c.color}20`}}>
                      <div style={{fontSize:12,fontWeight:800,color:c.color,marginBottom:5}}>{c.term}</div>
                      <div style={{fontSize:12,color:"#8892b0",lineHeight:1.65}}>{c.def}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Code example */}
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>💻 CODE EXAMPLE</div>
                <div style={{borderRadius:9,background:"#0a0c15",border:"1px solid #1a1d2e",overflow:"hidden"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 14px",background:"#0d0f1a",borderBottom:"1px solid #1a1d2e"}}>
                    <span style={{fontSize:11,color:lesson.color,fontFamily:"monospace",fontWeight:600}}>TypeScript</span>
                    <button onClick={() => navigator.clipboard.writeText(lesson.code)} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:5,fontSize:10,background:`${lesson.color}15`,border:`1px solid ${lesson.color}35`,color:lesson.color,cursor:"pointer"}}><Copy size={9}/>Copy</button>
                  </div>
                  <pre style={{margin:0,padding:"16px 18px",fontSize:12,lineHeight:1.8,fontFamily:"'JetBrains Mono','Fira Code',Consolas,monospace",color:"#c9d1f0",overflowX:"auto",whiteSpace:"pre"}}>
                    {lesson.code.split("\n").map((line,i) => (
                      <div key={i} style={{display:"flex"}}>
                        <span style={{width:30,flexShrink:0,color:"#2a2e46",userSelect:"none",textAlign:"right",paddingRight:12,fontSize:10}}>{i+1}</span>
                        <span dangerouslySetInnerHTML={{__html: highlight(line)||"&nbsp;"}}/>
                      </div>
                    ))}
                  </pre>
              ...
              {/* File Tree */}
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>📁 FOLDER STRUCTURE</div>
                <pre style={{margin:0,padding:"14px 18px",borderRadius:9,background:"#0a0c15",border:"1px solid #1a1d2e",fontSize:12,lineHeight:1.8,fontFamily:"'JetBrains Mono','Fira Code',Consolas,monospace",color:"#8892b0",overflowX:"auto",whiteSpace:"pre"}}>{lesson.fileTree}</pre>
              </div>

              {/* Stack Options */}
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>🔧 SWAPPABLE STACK OPTIONS</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {lesson.stacks.map((s,i) => (
                    <div key={i} style={{padding:"12px 16px",borderRadius:9,background:"#12141f",border:`1px solid ${lesson.color}18`}}>
                      <div style={{fontSize:11,fontWeight:800,color:lesson.color,marginBottom:7}}>{s.layer}</div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
                        {s.options.map((opt,oi) => (
                          <span key={oi} style={{fontSize:11,padding:"3px 10px",borderRadius:12,background:"rgba(255,255,255,0.04)",border:"1px solid #252840",color:"#9aa3c0"}}>{opt}</span>
                        ))}
                      </div>
                      <div style={{fontSize:11.5,color:"#4f8ef7",fontStyle:"italic"}}>✅ Recommended: {s.rec}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{marginBottom:28}}>
                <div style={{fontSize:10,fontWeight:800,color:"#5c6480",letterSpacing:"0.1em",marginBottom:10}}>🧠 QUIZ — CHECK YOUR UNDERSTANDING</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {lesson.quiz.map((q,qi) => {
                    const key = learnLesson * 100 + qi;
                    return (
                      <div key={qi} style={{borderRadius:9,background:"#12141f",border:`1px solid ${lesson.color}18`,overflow:"hidden"}}>
                        <div style={{padding:"12px 16px"}}>
                          <div style={{fontSize:13,fontWeight:700,color:"#eaedf8",marginBottom:8}}>Q{qi+1}: {q.q}</div>
                          <button onClick={() => setLearnQuiz(prev => ({...prev,[key]:!prev[key]}))} style={{
                            fontSize:11,padding:"5px 12px",borderRadius:7,cursor:"pointer",fontWeight:700,
                            background: learnQuiz[key] ? `${lesson.color}20` : "rgba(255,255,255,0.05)",
                            border: learnQuiz[key] ? `1px solid ${lesson.color}40` : "1px solid #252840",
                            color: learnQuiz[key] ? lesson.color : "#5c6480",
                          }}>{learnQuiz[key] ? "▾ Hide Answer" : "▸ Reveal Answer"}</button>
                          {learnQuiz[key] && (
                            <div style={{marginTop:10,padding:"10px 12px",borderRadius:7,background:`${lesson.color}08`,border:`1px solid ${lesson.color}20`}}>
                              <div style={{fontSize:12.5,color:"#c9d1f0",lineHeight:1.7}}>{q.a}</div>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {/* Navigation */}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderRadius:10,background:"#0a0c15",border:"1px solid #1a1d2e"}}>
                <button
                  onClick={() => setLearnLesson(Math.max(0, learnLesson - 1))}
                  disabled={learnLesson === 0}
                  style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,fontSize:12,fontWeight:700,cursor:learnLesson===0?"not-allowed":"pointer",opacity:learnLesson===0?0.3:1,background:"rgba(255,255,255,0.04)",border:"1px solid #252840",color:"#8892b0"}}
                >← Previous Lesson</button>

                <div style={{fontSize:12,color:"#5c6480"}}>
                  {learnLesson + 1} / {LESSONS.length}
                </div>

                {learnLesson < LESSONS.length - 1 ? (
                  <button
                    onClick={() => setLearnLesson(learnLesson + 1)}
                    style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer",background:`${LESSONS[learnLesson+1].color}18`,border:`1px solid ${LESSONS[learnLesson+1].color}40`,color:LESSONS[learnLesson+1].color}}
                  >Next: {LESSONS[learnLesson+1].title} →</button>
                ) : (
                  <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,fontSize:12,fontWeight:700,background:"rgba(52,211,153,0.12)",border:"1px solid rgba(52,211,153,0.3)",color:"#34d399"}}>
                    🎓 Course Complete!
                  </div>
                )}
              </div>
